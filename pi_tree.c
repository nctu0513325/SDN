...
int main(int argc, char **argv)
{
    ...
    while (step < world_size) {
        if (world_rank % (2 * step) == 0) {
            if (world_rank + step < world_size) {
                ...
                MPI_Recv(&received_count, 1, MPI_LONG_LONG_INT, world_rank + step, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                local_count += received_count;
            }
        } else {
            MPI_Send(&local_count, 1, MPI_LONG_LONG_INT, world_rank - step, 0, MPI_COMM_WORLD);
            break;
        }
        step *= 2;
    }
    ...
    {
        // TODO: PI result
        pi_result = 4.0 * local_count / tosses;
